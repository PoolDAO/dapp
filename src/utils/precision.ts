...
export const toPrecision = (input: number | string, precision: number = 18) => {
  ...
  if (ether === '.') {
    throw new Error(
      `while converting number ${input} with precision ${precision}, invalid value`
    )
  }
  ...
  if (comps.length > 2) {
    throw new Error(
      `while converting number ${input} with precision ${precision},  too many decimal points`
    )
  }
  ...
  if (fraction.length > baseLength) {
    throw new Error(
      `while converting number ${input} with precision ${precision}, too many decimal places`
    )
  }
